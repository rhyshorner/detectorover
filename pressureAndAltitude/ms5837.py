...
class MS5837(object):
    ...
    def __init__(self, model=MODEL_30BA, bus=1):
        ...
        try:
            ...
            print("Bus "+ str(bus) + "is not available.")
    ...
    def init(self):
        if self._bus is None:
            print("No bus!")
            return False
        if self._bus.write_byte(self._MS5837_ADDR, self._MS5837_RESET):
    ...
    def depth(self, barometric_pressure=101325):
        return (self.pressure(UNITS_Pa) - barometric_pressure) / (self._fluidDensity * 9.80665)
