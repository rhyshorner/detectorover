...
class ICM20948:
    ...
    def mag_data(self):
        self.magneto_data = list(self.read_magnetometer_data())
        return self.magneto_data
    ...
    def acc_gyr_data(self):
        data = list(self.read_accelerometer_gyro_data())
        middle_index = len(data)//2
        self.accel_data = data[:middle_index]
        self.gyro_data = data[middle_index:]
        return self.accel_data, self.gyro_data
    ...
    def computeOrientation(self):
        self.roll = np.arctan2(self.accel_data[1], self.accel_data[2] + 0.05*self.accel_data[0])
        self.pitch = np.arctan2(-1*self.accel_data[0], np.sqrt(np.square(self.accel_data[1]) + np.square(self.accel_data[2])))

        magLength = np.sqrt(np.square(self.magneto_data).sum())
        normMagVals = self.magneto_data/magLength
